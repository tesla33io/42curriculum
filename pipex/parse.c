...
/*   Updated: 2024/01/15 19:20:40 by astavrop         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	parse_fd(char **argv, t_pipex **data)
{
	(*data)->in_fd = open(argv[1], O_RDONLY);
	if ((*data)->in_fd < 0)
		return (print_error("Can't open \'INFILE\'", 41));
	(*data)->out_fd = open(argv[4], O_WRONLY);
	if ((*data)->out_fd < 0)
	{
		(*data)->out_fd = open(argv[4], O_WRONLY | O_CREAT, 00777);
		if ((*data)->out_fd < 0)
...
char	*check_cmd(char *cmd, char **paths)
{
	...
	free(bin);
	print_error("Command not found.", 6);
	return (NULL);
}
